#include "config.h"
      ...
      double precision, save :: mat(4,4), v(4), w(4)
...
!------------------------------------------------------------------------------
      ...
      do i=1,nfreq_input
        ...
        aux(4) = 1d0
        aux(5) = mode_list(i)%freq - dble(mode_list(i)%m)*Rota
...
!------------------------------------------------------------------------------
      ...
      aux(4) = 1d0
      aux(5) = mode_list(ifreq)%freq-dble(mode_list(ifreq)%m)*Rota
...
!------------------------------------------------------------------------------
       subroutine solve()
       implicit none
       integer, parameter ::  debut = 1, taille = 4
       double precision matrix(4,4), v_temp(4)
       double precision r
       integer icolumn, i, j
